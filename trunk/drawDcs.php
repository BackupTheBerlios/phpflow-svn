...
while ($offsety < $textareay - $fontheight / 2) {
  ...
  if ($offsety <= $textareay / 2 - $fontheight / 2) {
    ...
    $offsetx[$i] = ($textareax * 3/2) - (($offsety + $fontheight) 
                 * ($textareax / $textareay));
...
$points = array(0, floor($height / 2), 
                floor($width / 2), 0, 
                $width, floor($height / 2), 
                floor($width / 2), $height);
ImagePolygon($decision, $points, 4, $black);

$yvalue=0;
if ((strlen($textline[count($maxcharsperline)]) == '0') 
   && count($maxcharsperline) % 2 == '0') {
...
for($n = 0; $n <= $m; $n++) {
  ...
  if (strlen($textline[$n] == '0')) {
    ...
    log2file('[drawDcs] yval[$n]+yvalue+yoffset: '.
              $yval[$n].'+'.$yvalue.'+'.$yoffset);
    log2file('[drawDcs] textline[$n]: $textline[$n]');
    ImageString($decision, $font, $xval, $yval[$n] + $yvalue + $yoffset, 
                $textline[$n], $black);
    $yvalue = 0;
  }
}
