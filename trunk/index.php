...
if(!$ini_data = @parse_ini_file($PHPFLOW["INI_DIR"].$file, TRUE))
{
...
while (list($row_nr, $row_data) = each($ini_data))
{
	if (strstr($row_nr, "row"))
	{
		...
		do 
		{
			...
			$flowchart .= "<td align=\"center\"><a name=\"row".$rowpos."col".$column."\"".$linksrc.">";
			switch ($type)
			{
				case "Arrow":
					...
					$flowchart .= "<img src=\"drawArr.php?w=$width&amp;h=$arrow_space&amp;s=$start&amp;e=$end&amp;f=$font&amp;t=$text\"$noborder alt=\"Arrow: '$start-$end'\">";		
				break;
				case "ArrowDown":
					$text = rawurlencode($row_data["text".$column]);
					$flowchart .= "<img src=\"drawArr.php?w=$width&amp;h=$arrow_space&amp;s=top&amp;e=bottom&amp;f=$font&amp;t=$text\"$noborder alt=\"Arrow: 'top-bottom'\">";		
				break;
				case "ArrowLeft":
					$text = rawurlencode($row_data["text".$column]);
					$flowchart .= "<img src=\"drawArr.php?w=$arrow_space&amp;h=$arrow_space&amp;s=right&amp;e=left&amp;f=$font&amp;t=$text\"$noborder alt=\"Arrow: 'right-left'\">";		
				break;
				case "ArrowRight":
					$text = rawurlencode($row_data["text".$column]);
					$flowchart .= "<img src=\"drawArr.php?w=$arrow_space&amp;h=$arrow_space&amp;s=left&amp;e=right&amp;f=$font&amp;t=$text\"$noborder alt=\"Arrow: 'left-right'\">";		
				break;
				case "Decision":
					$text = rawurlencode($row_data["text".$column]);
					log2file("[ini-wrapper] Dcs-text: ".$text);
					$flowchart .= "<img src=\"drawDcs.php?w=$width&amp;h=$height&amp;f=$font&amp;t=$text\"$noborder alt=\"Decision: '$text'\">";
				break;
				case "Input/Output":
					$text = rawurlencode($row_data["text".$column]);
					log2file("[ini-wrapper] IO-text: ".$text);
					$flowchart .= "<img src=\"drawIO.php?w=$width&amp;h=$height&amp;f=$font&amp;t=$text\"$noborder alt=\"Input/Output: '$text'\">";
				break;
				case "Line":
					$start = $row_data["start".$column];
					$end = $row_data["end".$column];
					$text = rawurlencode($row_data["text".$column]);
					$flowchart .= "<img src=\"drawLine.php?w=$width&amp;h=$height&amp;s=$start&amp;e=$end&amp;f=$font&amp;t=$text\"$noborder alt=\"Line: '$start-$end'\">";		
				break;
				case "LineShort":
					$start = $row_data["start".$column];
					$end = $row_data["end".$column];
					$text = rawurlencode($row_data["text".$column]);
					$flowchart .= "<img src=\"drawLine.php?w=$arrow_space&amp;h=$arrow_space&amp;s=$start&amp;e=$end&amp;f=$font&amp;t=$text\"$noborder alt=\"Line(short): '$start-$end'\">";		
				break;
				case "LoopBegin":
					$text = rawurlencode($row_data["text".$column]);
					log2file("[ini-wrapper] LpB-text: ".$text);
					$flowchart .= "<img src=\"drawLpB.php?w=$width&amp;h=$height&amp;f=$font&amp;t=$text\"$noborder alt=\"Loop Begin: '$text'\">";
				break;
				case "LoopEnd":
					$text = rawurlencode($row_data["text".$column]);
...
// fill template
$tmpl = fopen($PHPFLOW["TEMPLATE"],"r");
if(!$tmpl)
{
	die("could not open ".$PHPFLOW["TEMPLATE"]."!");
}
$tmplsource = fread($tmpl,filesize($PHPFLOW["TEMPLATE"])); 
fclose($tmpl);
